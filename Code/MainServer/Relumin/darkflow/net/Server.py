...
from darkflow.net.utils.inference import draw_text
from darkflow.net.utils.inference import draw_bounding_box
...
def main():
    ...
    global tfnet, emotion_window,frame_window,emotion_labels,emotion_classifier,emotion_target_size,emotion_offsets,face_detection

    tfnet = TFNet(options)  # 객체를 하나 만들어주기 option[Model, weights]값을 넣어줘서 선언.
    pytesseract.pytesseract.tesseract_cmd = 'C:/Program Files (x86)/Tesseract-OCR/tesseract' #테서렉트 엔진 경로.
    ...
    while True:
        ...
        while True:
            ...
            inputNumber = (conn.recv(1024)).decode('utf-8')  # 웨어러블 디바이스로 부터 기능번호를 받음 1.object 2. Text 3.Emotion

            print('---------입력받은 기능번호 1.Object 2.Text 3.Emotion : {}--------- '.format(inputNumber))
            ...
            if length == None:  # client에서 강제종료되었을 때
                break
